...
function! s:PHPCustomFolds() " {{{
	...
	" call s:PHPFoldPureBlock('^\s*function', s:FOLD_WITH_PHPDOC)

    " Fold function with PhpDoc (function foo() {})
    " Don't fold abstract functions because it's annoying.
    call s:PHPFoldPureBlock('\(\s*\<abstract\s\+.*\)\@<!function', s:FOLD_WITH_PHPDOC)

	" Fold class properties with PhpDoc (var $foo = NULL;)
	call s:PHPFoldProperties('^\s*\(static\s\+\)\?\(private\|protected\|public\)\s\+\s\$', ";", s:FOLD_WITH_PHPDOC, 1, 1)
...
function! s:PHPFoldPureBlock(startPattern, ...) " {{{
	...
	while 1
		...
		if s:lineStart != 0
            ...
            " Don't fold empty blocks, it's annoying. 
