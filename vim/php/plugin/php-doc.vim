...
let g:pdv_cfg_Author = "Tina Gunnarsson <tina.gunnarsson@webex.com>"
let g:pdv_cfg_Copyright = "2011 Cisco"
...
" let g:pdv_re_attribute = '^\s*\(\(private\|public\|protected\|const\|static\)\+\)\s*\$\([^ ;=]\+\)[ =]*\(.*\);\?$'
let g:pdv_re_attribute = '\v\s*(%(const )|%([a-zA-Z ]*)\$)([^\s;=]+)%(\s*\=\s*(.*))?;'
...
func! PhpDocVar()
	...
	let l:varname = substitute (l:name, g:pdv_re_attribute, '\2', "g")
	let l:default = substitute (l:name, g:pdv_re_attribute, '\3', "g")

    let l:scope = PhpDocScope(l:modifier, l:varname)

    let l:static = matchstr(l:modifier, g:pdv_re_static)
    let l:abstract = matchstr(l:modifier, g:pdv_re_abstract)
    let l:const = matchstr(l:modifier, g:pdv_re_const)
    ...
    if l:const != ""
        exe l:txtBOL . g:pdv_cfg_Commentn . "@constant" . g:pdv_cfg_EOL
    endif
